...
# Build helper: packages a Python fuzzer with pyinstaller and creates a
# sanitizer-aware execution wrapper.
build_fuzzer() {
  local fuzzer=$1
  local fuzzer_basename
  fuzzer_basename=$(basename -s .py "$fuzzer")
  local fuzzer_package=${fuzzer_basename}.pkg
...
# Build the original coverage fuzzer.
build_fuzzer coverage_atheris_fuzzer.py

# Build the advanced configurable coverage fuzzer.
build_fuzzer advanced_coverage_atheris_fuzzer.py
